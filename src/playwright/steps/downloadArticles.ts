...
export async function downloadArticlesStep(context: BrowserContext, journalPageUrl: string, journalName: string) {
  ...
  const realJournalName = (await page.$eval('.title', el => el.textContent?.trim() || journalName)).trim();
  const sanitizedJournal = sanitizeName(realJournalName);

  // Extract volume/year/month from the first .volume div (before clicking)
  const volumeDiv = await page.$('div.volume');
  let volumeText = '';
  ...
  if (volumeDiv) {
    ...
    // Try to extract year, volume, and month from the text (e.g., "Volume 123, Number 4, Winter 2024")
  ...
  for (const link of articleLinks) {
    ...
    if (href) {
      ...
      try {
        ...
        // Build the folder structure: downloads/<journal>/<year>/<volume>/<month>/
